(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.currency = "RM"; // HTML entity of the currency to be displayed in the layout
			this.currencyString = "RM"; // Currency symbol as textual string
			this.paypalCurrency = "RM"; // PayPal's currency code
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						var item = items[i];
						var product = item.product;
						var price = this.currency + " " + item.price.toFixed(2);
						var qty = item.qty;
				...
				if( cartItems.length > 0 ) {
					...
					for( var j = 0; j < cartItems.length; ++j ) {
						...
						var cartPrice = this.currency + " " + cartItem.price.toFixed(2);
